...
export default function Sidebar({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const mainNavItems = [
    ...
    { name: "Document Hub", href: "/documents", icon: Files },
  ];
  ...
  const NavItem = ({ href, icon: Icon, name }: { href: string; icon: React.ElementType; name: string }) => {
    ...
    return (
      <Link
        ...
        onClick={handleNav}
        className={cn(
          "w-full flex items-center gap-3 px-3 py-2 rounded-xl text-sm transition-colors text-left font-medium",
          isActive
  ...
  return (
    <>
      ...
      <aside className={cn(
        "w-60 bg-white border-r border-zinc-200 flex flex-col h-full flex-shrink-0 z-40",
        ...
        {/* Logo */}
        <div className="h-14 flex items-center px-4 border-b border-zinc-200 flex-shrink-0 justify-between">
        ...
        <div className="px-4 py-3 border-t border-zinc-100 flex-shrink-0">
          <div className="flex items-center justify-between text-zinc-400">
            <div className="flex items-center gap-1.5">
              ...
              <p className="text-[10px] font-semibold tracking-wide">Live</p>
            </div>
            <p className="text-[10px] font-mono bg-zinc-100 text-zinc-500 px-1.5 py-0.5 rounded">
